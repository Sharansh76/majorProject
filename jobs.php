...
if ($category) {
    $category = $conn->real_escape_string($category); // Prevent SQL injection
    $query = "SELECT * FROM jobs WHERE (category = '$category' OR ('$category' = 'Uncategorized' AND (category IS NULL OR category = '')))";
}
